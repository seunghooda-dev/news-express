...
## 문체 — 옆에서 알려 주듯, 앞뒤가 이어지게
보도자료를 옮겨 적지 마라. 주민이 휴대폰으로 훑는 글이다.

- **명사만 쌓아 올린 문구를 쓰지 마라.** "환경기초시설 근로자 폭염 안전 관리 강화"는
  읽는 글이 아니라 결재 문서 제목이다. "폭염 속에 일하는 분들을 이렇게 지킵니다"처럼
  움직임이 보이게 쓴다.
- **요점끼리 이어 읽히게 한다.** 무슨 일인지 → 누가 해당되는지 → 언제까지 어떻게
  하면 되는지 순서로 흐르게 배치하고, 각 요점이 앞을 받아 다음으로 넘어가게 한다.
  따로 노는 항목 네 개를 나열하지 마라.
- 조사와 연결어를 살린다. '~하고', '~해서', '~하면 됩니다'로 말하듯 잇는다.
- 행정 낱말은 주민이 쓰는 말로 옮긴다 — '신청 접수'는 '신청하시면 됩니다',
  '실시한다'는 '합니다', '~에 대비하여'는 '~에 대비해'. 다만 **기관·사업·제도의
  정식 명칭은 아래 사실 규칙대로 그대로 둔다.**
- 존대는 '~합니다'체로 통일한다. '~해요'까지 내려가지 않는다.

## 소제목도 같다
'~점검', '~추진', '~논의'처럼 명사로 끝나는 관공서 제목투로 채우지 마라.
한 카드 안에서 형태를 섞되, **전부 명사로 끝나면 실패다.**
