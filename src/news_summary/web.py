...
def create_app() -> Flask:
    ...
    def add_security_headers(response: Response):
        ...
        # app.css처럼 ?v=<커밋>이 붙은 정적 파일도 같은 이유로 캐시한다 — 배포하면
        # 주소가 바뀌므로 낡은 파일이 남지 않는다(2026-08-05).
        if request.path.startswith("/static/fonts/") or (
            request.path.startswith("/static/") and request.args.get("v")
        ):
    ...
    def preview_press_release_asset(asset_id: int):
        ...
        with _asset_preview_fetch_limit:
            ...
            # 원본을 펼쳤던 메모리는 여기서 이미 free됐지만 glibc가 쥐고 있다 — 돌려준다.
            _release_free_heap()
...
def _release_free_heap() -> bool:
    """썸네일을 만든 뒤 해제된 힙을 OS에 돌려준다(2026-08-06 실측 대응).

    원본을 펼쳤다 줄이는 과정에서 잡힌 메모리를 파이썬이 free해도 glibc가 arena에
    쥐고 있어 RSS가 안 내려간다. 실측으로 썸네일 1건당 약 1MB가 남았고, 새 기사가
    들어오는 낮 동안 4시간에 +71MB씩 올라 하루면 512MB 한도에 닿았다.
    밤에는 새 기사가 없어 11시간 동안 ±1MB로 완전히 평평했다 — 즉 생성 경로가
    유일한 상승원이다.

    glibc가 아니면(윈도우·musl) 조용히 넘어간다. 되돌리려면 이 호출만 지우면 된다.
    """
    ...
    try:
        trim(0)
    except Exception as exc:  # noqa: BLE001 - 메모리 반납 실패가 이미지 응답을 막으면 안 된다.
