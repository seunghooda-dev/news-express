    ...
    try:
        with target.connect() as conn:
            if replace:
                conn.execute(
                    """
                    TRUNCATE source_collection_runs, draft_history, article_drafts, app_metadata, press_releases
...
def _reset_sequences(conn: Any) -> None:
    for table in ("press_releases", "article_drafts", "draft_history", "source_collection_runs"):
        conn.execute(
            f"""
            SELECT setval(
